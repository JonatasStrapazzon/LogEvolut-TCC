...
class SplitterAttributes extends AbstractAttributeSet {
	...
	public <V> void setValue(Attribute<V> attr, V value) {
		if (attr == StdAttr.FACING) {
			...
			configureOptions();
			parameters = null;
		} else if (attr == ATTR_FANOUT || attr == ATTR_WIDTH) {
                    
                        int newValue;
                        if(value instanceof BitWidth) newValue = ((BitWidth)value).getWidth();
                        else newValue = ((Integer) value).intValue();
			...
			if (bit_end.length != newValue)
                            bit_end = new byte[newValue];
