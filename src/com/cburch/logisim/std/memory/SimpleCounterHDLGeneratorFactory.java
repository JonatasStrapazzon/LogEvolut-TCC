...
public class SimpleCounterHDLGeneratorFactory extends AbstractHDLGeneratorFactory {
	...
	public SortedMap<String, Integer> GetParameterMap(Netlist Nets,
		...
		int mode = 0;
		if (attrs.containsAttribute(SimpleCounter.ATTR_ON_GOAL)) {
			if (attrs.getValue(SimpleCounter.ATTR_ON_GOAL) == SimpleCounter.ON_GOAL_STAY)
				mode = 1;
			else if (attrs.getValue(SimpleCounter.ATTR_ON_GOAL) == SimpleCounter.ON_GOAL_CONT)
				mode = 2;
			else if (attrs.getValue(SimpleCounter.ATTR_ON_GOAL) == SimpleCounter.ON_GOAL_LOAD)
				mode = 3;
		} else {
			mode = 1;
		}
		...
		ParameterMap
				.put(MaxValStr, attrs.getValue(SimpleCounter.ATTR_MAX).intValue());
		int ClkEdge = 1;
		if (GetClockNetName(ComponentInfo, SimpleCounter.CK, Nets).isEmpty()
	...
	public SortedMap<String, String> GetPortMap(Netlist Nets,
		...
		AttributeSet attrs = ComponentInfo.GetComponent().getAttributeSet();
		if (!ComponentInfo.EndIsConnected(SimpleCounter.CK)) {
			Reporter.AddSevereWarning("Component \"SimpleCounter\" in circuit \""
					+ Nets.getCircuitName() + "\" has no clock connection");
			PortMap.put("GlobalClock", ZeroBit);
			PortMap.put("ClockEnable", ZeroBit);
		} else {
			String ClockNetName = GetClockNetName(ComponentInfo, SimpleCounter.CK,
					Nets);
			if (ClockNetName.isEmpty()) {
				PortMap.putAll(GetNetMap("GlobalClock", true, ComponentInfo,
						SimpleCounter.CK, Reporter, HDLType, Nets));
		...
		if (HDLType.equals(Settings.VHDL)
			...
			Input += "(0)";
		PortMap.putAll(GetNetMap(Input, true, ComponentInfo, SimpleCounter.IN,
				Reporter, HDLType, Nets));
		PortMap.putAll(GetNetMap("clear", true, ComponentInfo, SimpleCounter.CLR,
				Reporter, HDLType, Nets));
		PortMap.putAll(GetNetMap("load", true, ComponentInfo, SimpleCounter.LD,
				Reporter, HDLType, Nets));
		PortMap.putAll(GetNetMap("Enable", false, ComponentInfo, SimpleCounter.EN,
				Reporter, HDLType, Nets));
		PortMap.putAll(GetNetMap("Up_n_Down", false, ComponentInfo, SimpleCounter.UD,
				Reporter, HDLType, Nets));
		...
		if (HDLType.equals(Settings.VHDL)
				& (ComponentInfo.GetComponent().getAttributeSet()
						.getValue(StdAttr.WIDTH).getWidth() == 1))
			Output += "(0)";
		PortMap.putAll(GetNetMap(Output, true, ComponentInfo, SimpleCounter.OUT,
				Reporter, HDLType, Nets));
		PortMap.putAll(GetNetMap("CompareOut", true, ComponentInfo,
				SimpleCounter.CARRY, Reporter, HDLType, Nets));
		return PortMap;
	}
